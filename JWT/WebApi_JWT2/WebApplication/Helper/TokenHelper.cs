...
namespace WebApplication.Helper
{
    public static class TokenHelper
    {
        ...
        /// <returns></returns>
        public static string GenerateToken(AuthInfo info, int expireMinutes = 120)
        {
            // 生成二进制字节数组
            ...
            {
                //想要放进去的内容（此部分将来为类似为明文，所以不能加入敏感信息比如密码类）
                Subject = new ClaimsIdentity(new[] // Token的身份证，类似一个人可以有身份证，户口本
                        {
                            // 可以创建多个
                            new Claim(ClaimTypes.Name, info.UserName), 
                            new Claim(ClaimTypes.MobilePhone,info.UserPhone),
        ...
        #region 此方法用解码字符串token，并返回秘钥的信息对象
        /// <summary>
